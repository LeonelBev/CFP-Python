...
def colors ():
    ...
    color = colores.get(num, "Error. Operación no existente.")
    return(color)
...
class SnakeApp:
    def __init__(self, root):
        ...
        self.canvas = tk.Canvas(root, width=ANCHO * TAMANO_CELDA, height=ALTO * TAMANO_CELDA, bg=colorsbg())
        self.canvas.pack()
    ...
    def iniciar_juego(self):
        ...
        self.direccion = (0, 0)  # Iniciar moviéndose hacia abajo
    ...
    def actualizar_tablero(self):
        ...
        self.canvas.config(bg=colorsbg())
        
        for x, y in self.snake:
            self.canvas.create_rectangle(x * TAMANO_CELDA, y * TAMANO_CELDA,
                                         (x + 1) * TAMANO_CELDA, (y + 1) * TAMANO_CELDA, fill=colors())
